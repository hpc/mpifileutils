...
#ifdef DAOS_SUPPORT
static void daos_check_path(int* rank, struct duns_attr_t dattr,
                            const char** argpaths, uuid_t* pool_uuid,
                            uuid_t* cont_uuid, mfu_file_t* mfu_file)
{
...
int main (int argc, char** argv)
{
	...
	/* DAOS specific options */
        { "pool"                , required_argument, 0, 'x' },
        { "cont"                , required_argument, 0, 'y' },
    ...
    if (!daos_uuid_valid(pool_uuid)) {
        daos_check_path(&rank, dattr, argpaths, &pool_uuid, &cont_uuid, mfu_file);
