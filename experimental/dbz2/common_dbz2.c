...
void DBz2_Dequeue(CIRCLE_handle* handle)
{
    ...
    /*compress block*/
    ret = BZ2_bzBuffToBuffCompress(a[blocks_processed], &outSize, ibuf, inSize, bc_size, 0, 30);
...
void DBz2_decompEnqueue(CIRCLE_handle* handle)
{
    ...
    /*The function reads the trailer, which stores the offset of all the blocks and creates a string of the form:
    ...
    while (1) {
        if (end == trailer_begin)
            /*when the offset of the trailer is read, all block offsets have been read and queued*/
        {
            break;
        }
...
void DBz2_decompDequeue(CIRCLE_handle* handle)
{
    ...
    /*parse neop to get block_no, offset and offset of next block*/
    ...
    ret = BZ2_bzBuffToBuffDecompress(obuf, &outSize, ibuf, length, 0, 0);
