...
class test(object):
	...
	def __init__(self):
		...
		os.environ['PATH'] = self.dtar_path+'/install/include:'+os.environ['PATH']
		os.environ['LD_LIBRARY_PATH'] = self.dtar_path+'/install/lib:'+os.environ['LD_LIBRARY_PATH']
	...
	def main(self):	
		...
		if self.run_test:
			...
			if not os.path.exists(self.bin_path):
				print '%s:\tno such file' %self.bin_path
				sys.exit(2)

		if self.tmp_dir == '': self.tmp_dir = self.cwd + '/tmp'
		
		self.dest_dir = self.cwd + '/dest'
		...
		if self.titan:
			...
			self.job.write('#PBS -l walltime=1:00:00,nodes=%s,%s\n' %(str(self.num_proc/self.procpernode) if self.num_proc%self.procpernode == 0 else str(self.num_proc/self.procpernode+1), ('partition=dtn' if self.dtn else '')))
			self.job.write('#End PBS directives\n')
			self.job.write('cd %s\n' %self.cwd)
			self.job.write('export PATH=$PATH:/lustre/widow2/scratch/tag/bayer/dtar/install/include\n')
			self.job.write('export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/lustre/widow2/scratch/tag/lib\n')
